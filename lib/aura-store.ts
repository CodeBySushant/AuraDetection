...
export interface AuraResult {
  ...
  // optional fallback (if you still use it somewhere)
  imageUrl?: string;
}
...
export const useAuraStore = create<AuraStore>((set) => ({
  ...
  reset: () =>
    set({
      formData: { ...initialFormData }, // ✅ new object
      result: null,
      isLoading: false,
    }),
}));

export function getAuraImageSrc(result: AuraResult) {
  if (result.image) {
...
export const auraColorMap: Record<
  string,
...
export function generateAuraDescription(
  colors: string[],
  name: string,
  mood: string,
  personality: string,
